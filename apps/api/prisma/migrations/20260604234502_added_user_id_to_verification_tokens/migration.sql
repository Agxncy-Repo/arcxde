/*
  ...
  - You are about to alter the column `user_id` on the `verification_tokens` table. The data in that column could be lost. The data in that column will be cast from `Text` to `VarChar(255)`.
...
*/
-- DropIndex
DROP INDEX "verification_tokens_user_id_idx";
...
-- AlterTable
ALTER TABLE "verification_tokens" ADD COLUMN     "email" VARCHAR(254) NOT NULL,
ALTER COLUMN "id" SET DEFAULT 'vt_' || gen_random_uuid()::text,
