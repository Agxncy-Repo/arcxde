/*
  ...
  - A unique constraint covering the columns `[registrationToken]` on the table `email_verification_sessions` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `registrationToken` to the `email_verification_sessions` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "email_verification_sessions" ADD COLUMN     "registrationToken" TEXT NOT NULL,
ALTER COLUMN "id" SET DEFAULT 'evs_' || gen_random_uuid()::text;
...
-- CreateIndex
CREATE UNIQUE INDEX "email_verification_sessions_registrationToken_key" ON "email_verification_sessions"("registrationToken");
